...
function UpcomingMatches() {
  ...
  const showHandle = (item) => {
    ...
    const formattedTime =
      ...
      `:${minutes < 10 ? '0' : ''}${minutes} ${amOrPm}`;

    item = { ...item, date: formattedDate, time: formattedTime }
    setItemData(item)
    setModal(true)
  }
                        ...
                        {matches.map((item) => (
                          <div className="mt-2 " key={item._id}>
                            <div className="relative flex flex-col justify-end overflow-hidden rounded-b-xl pt-6">
                              <div className="group relative flex cursor-pointer justify-between rounded-xl bg-white before:absolute before:inset-y-0 before:right-0 before:w-1/2 before:rounded-r-xl before:bg-gradient-to-r before:from-transparent before:to-green-400 before:opacity-0 before:transition before:duration-500 hover:before:opacity-100">
                                <div className="relative space-y-1 p-6">
                                  <h4 className="text-lg text-black">Match No: {item.matchnumber}</h4>
                                  <div className="relative h-20 text-green-500 text-lg">
                                    <div className='flex'>
                                    ...
                                    <button href="" class="mt-10 mx-1  flex items-center gap-1 w-32 invisible absolute left-0 top-0 translate-y-3 transition duration-300 group-hover:visible group-hover:translate-y-0" onClick={() => { showHandle(item) }}>
                                      <span className='text-yellow-500'>View now </span>
      ...
      {modal && (
        <div className="fixed inset-0 flex items-center justify-center z-30 bg-black bg-opacity-50" >
          <div className="bg-white p-6 rounded-lg z-10">
            ...
            <h2 className="text-2xl font-bold mb-4 text-center">{itemData.tournament.tournamentname}</h2>

            <div class="group relative flex cursor-pointer justify-center rounded-xl bg-green-400 before:absolute before:inset-y-0 before:right-0 before:w-1/2 before:rounded-r-xl before:bg-gradient-to-r before:from-transparent before:to-white before:opacity-0 before:transition before:duration-500 hover:before:opacity-100 w-[20rem] h-[26rem] md:w-[32rem] md:h-[32rem]">
              <div class="relative  space-y-1 p-4">
                ...
                <div className='pt-6'>
                  <h4 class=" text-blue-700 font-normal text-xl justify-center text-center">Date :- <span className='text-black text-xl'> {itemData.date}</span></h4>
                  <h4 class=" text-blue-700 font-normal text-xl justify-center text-center">Time :- <span className='text-black text-xl'> {itemData.time}</span></h4>
                  <h4 class=" text-blue-700 font-normal text-xl justify-center text-center">Tickets :- <span className='text-black text-xl'> {itemData.tickets}</span></h4>
                  <h4 class=" text-blue-700 font-normal text-xl justify-center text-center">Tickets Fee :- <span className='text-black text-xl'> {itemData.ticketsfee}</span></h4>
                  <h4 class=" text-blue-700 font-normal text-xl justify-center text-center">status :- <span className='text-black text-xl'> {(itemData.matchstatus === true ? 'upcoming' : 'Over')}</span></h4>
